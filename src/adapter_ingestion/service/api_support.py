...
def _enforce_auth_context(
    ...
    *,
    ...
    if require_study_research and not bearer_token:
        raise HTTPException(status_code=401, detail="Bearer token required for study research operation")
    ...
    if bearer_token:
        try:
            session_claims = validate_session_access_token(
                bearer_token,
                settings,
                force_secure=require_study_research,
            )
            ...
            if require_study_research:
                ...
                if not actor or actor != subject:
                    raise HTTPException(status_code=403, detail="Bearer token research actor binding is invalid")
                payload_issuer = _extract_iss(payload)
                if payload_issuer and payload_issuer != actor:
                    raise HTTPException(status_code=403, detail="DIDComm issuer does not match Bearer token research actor")
