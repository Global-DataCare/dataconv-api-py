...
def register_organization_tenant_routes(app: Any, *, activation_manager: Any) -> None:
    @app.post(
        "/publisher/cds-{jurisdiction}/v1/{sector}/{tenant_id}/organization/tenant/_status",
        tags=["1.3 Organization Tenant Activation"],
        summary="Check DataConv tenant readiness with a current ICA controller proof",
        description=(
            "Returns not-configured when the scoped tenant record does not exist or is not active. "
            "The caller may then retry the idempotent _activate operation."
        ),
    )
    async def get_organization_tenant_status(
        jurisdiction: str,
        sector: str,
        tenant_id: str,
