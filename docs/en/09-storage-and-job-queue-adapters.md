...
New jobs in the explicit `onehealth-research` and `animal-research` sectors
persist one stable FHIR `ResearchStudy` Reference from upload through review.
Poll and patch requests must repeat it, and promoted
...
local authorization rule. GW remains responsible for evaluating DCR and the
active Consent, then DataConv validates its signed study SMART token offline
and enforces the derived actor/study token without querying GW per operation. Jobs
